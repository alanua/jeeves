...
Do not confuse aliases:

```text
СК = Skeleton / ChatGPT exoskeleton
ДЖ = Jeeves project
СТ СК is not Jeeves startup.
СТ ДЖ is Jeeves startup.
```
...
- explanations matched to the user's basic programming level unless the user asks for expert depth
...
- when the user asks to adjust behavior, treat it as a candidate durable rule: check existing rules, avoid duplicates/conflicts, then save only the cleaned rule
