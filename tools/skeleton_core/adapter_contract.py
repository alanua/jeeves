...
Side-effect-free contract between Skeleton and executor adapters.
It does not run tools, create branches, open PRs, merge, deploy, read secrets,
or access production systems.
"""
...
class AdapterTaskPacket(BaseModel):
    """Bounded input packet sent from Skeleton to an executor adapter."""
...
def _forbidden_path_block_reasons(value: str) -> list[str]:
    """Validate forbidden path scopes.

    Secret/control paths are valid here because this list is used to forbid them.
    """
    normalized = _normalize_path(value)
    reasons: list[str] = []

    if normalized != value:
        reasons.append("forbidden_path_contains_backslash")
    if normalized in ROOT_OR_SHORTCUT_TARGETS:
        reasons.append("forbidden_path_is_root_or_shortcut")
    if normalized.startswith("/"):
        reasons.append("forbidden_path_is_absolute_path")
    if _is_path_traversal(normalized):
        reasons.append("forbidden_path_contains_path_traversal")
    if _has_wildcard(normalized):
        reasons.append("forbidden_path_contains_wildcard")
...
def validate_task_packet(
    ...
    for forbidden_path in forbidden_paths:
        blocked.extend(_forbidden_path_block_reasons(forbidden_path))
